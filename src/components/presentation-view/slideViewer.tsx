...
const useSlideStateManager = () => {
  ...
  const updateSlideState = useCallback((outlineTitle: string, updates: Partial<SlideState>) => {
    setSlideStates(prev => ({
      ...prev,
      [outlineTitle]: {
        ...prev[outlineTitle],
...
export default function ViewPresentation() {
  ...
  const { slideStates, updateSlideState, initializeSlideStates } = useSlideStateManager();
      ...
      case "slides":
        return (
          <>
            ...
            {!shouldShowLoading() && !shouldShowError() && slideState.genSlideID && (
              <iframe
          ...
          <NewSlideVersion
            ...
            handleCustomBuilderClick={() => {
              if (featureDisabled) {
                ...
                const newMode = outlineTitle === outlines[0].title ? "Cover" :
                               outlineTitle === outlines[outlines.length - 1].title ? "Contact" :
                               "customBuilder";
                setDisplayModes(prev => ({
                  ...prev,
                  [outlineTitle]: newMode
...
useEffect(() => {
  if (currentOutline !== "" && documentID !== null && !slidesArray[currentOutline]) {
    ...
    <div className="flex flex-col lg:flex-row bg-[#F5F7FA] h-full md:h-[100vh] no-scrollbar no-scrollbar::-webkit-scrollbar">
      ...
      <div className="hidden lg:flex lg:flex-row lg:w-full lg:pt-16 ">
        ...
        <div className="flex-1">
          ...
          >
            {slideStates[currentOutline]?.isLoading && !slideStates[currentOutline]?.isNoGeneratedSlide && (
              <div className="w-full h-full flex flex-col gap-y-3 items-center justify-center">
              <div className="w-10 h-10 border-4 border-t-blue-500 border-gray-300 rounded-full animate-spin"></div>
              <h1>Generating Slide Please Wait...</h1>
              {slideStates[currentOutline]?.retryCount > 0 && (
                <p className="text-sm text-gray-500">
                Retry attempt {slideStates[currentOutline].retryCount}/3...
                </p>
              )}
              </div>
            )}
