...
export default function Graphs({
  ...
  const validateData = () => {
    ...
    const areHeadersValid =
      headers[0] && headers[0].trim() !== "" && headers[1] && headers[1].trim() !== "";
  
    // Check if required fields in rows are filled
    const isRowValid = rows.every((row) => row.label.trim() !== "" && row.services.trim() !== "");
    ...
    setIsButtonDisabled(
      !(isSlideTitleValid && isSlideTypeValid && hasMinimumRows && areHeadersValid)
    );
  ...
  <button
    ...
    className={`flex-1 lg:flex-none lg:w-[180px] py-2 rounded-md transition-all duration-200 transform ${
      isButtonDisabled ? "bg-gray-200 text-gray-500" : "bg-[#3667B2] text-white"
    }`}
  >
    Generate Slide
    {/* Tooltip */}
