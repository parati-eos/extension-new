...
export default function Graphs({
  ...
  const [rows, setRows] = useState<Row[]>([{ label: '', services: '', series3: '' }]);
  const [series, setSeries] = useState(1)
  const [isButtonDisabled, setIsButtonDisabled] = useState(true)
  const [isAddRowDisabled, setIsAddRowDisabled] = useState(true)
  const [isLoading, setIsLoading] = useState(false)
  const tableRef = useRef<HTMLDivElement | null>(null)
  const [columns, setColumns] = useState<string[]>(['Series 1', 'Series 2', 'Series 3']);
    ...
    try {
      const seriesData = columns.map((column, index) => {
        const key =
          index === 0
            ? "label"
            : `series${index}`; // Use series1, series2, series3 dynamically
        return {
