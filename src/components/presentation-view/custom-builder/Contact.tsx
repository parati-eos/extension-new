...
export default function Contact({
  ...
  const validateWebsiteLink = () => {
    ...
    if (websiteLink && !regex.test(websiteLink)) {
      setErrors((prev) => ({ ...prev, websiteLink: 'Enter a valid website URL' }))
    } else {
  ...
  const validatePhone = () => {
    ...
    if (phone && !regex.test(phone)) {
      setErrors((prev) => ({ ...prev, phone: 'Enter a valid phone number (10 digits)' }))
    } else {
  ...
  const validateLinkedin = () => {
    const regex = /^https:\/\/(www\.)?linkedin\.com\/(in|company|pub)\/[a-zA-Z0-9_-]{3,}\/?$/
    if (linkedin && !regex.test(linkedin)) {
      setErrors((prev) => ({ ...prev, linkedin: 'Enter a valid LinkedIn profile URL' }))
    } else {
